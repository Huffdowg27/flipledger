...
export default function SupplierProfitabilityPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/data/profitability?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}${marketplaceParam}&groupBy=supplier`);
  ...
  const columns = useMemo<ColumnDef<ProfitRow, any>[]>(() => [
    {
      ...
      cell: ({ getValue }) => <span className="text-sm text-text-primary font-medium">{getValue() as string || 'Unknown'}</span>,
      size: 200,
    },
    {
      id: 'orders',
  ...
  function handleExport() {
    const headers = ['Supplier', 'Orders', 'Units Sold', 'Refunds', 'Revenue', 'Cost/Unit', 'Fees', 'On Hand', 'Profit', 'ROI %', 'Margin %'];
    const csvRows = rows.map(r => [
      `"${r.groupKey}"`, r.orders, r.unitsSold, r.refunds,
    ...
    a.href = url;
    a.download = 'supplier-profitability.csv';
    a.click();
    URL.revokeObjectURL(url);
  }

  if (loading) return <ProfitSkeleton />;

  const footerRow = totals ? {
    supplier: <span className="text-text-primary font-semibold">Totals</span>,
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Analyze > Supplier Profitability"
        dateRange={dateRange}
