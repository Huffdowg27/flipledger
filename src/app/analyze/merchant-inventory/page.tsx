...
import { Search, Printer, X, Check, CheckCircle2, RefreshCw, Copy, Download, Eye, Send, Loader2, AlertTriangle } from 'lucide-react';
...
// ---------------------------------------------------------------------------
// PreviewModal — activation preview + push flow.
// ---------------------------------------------------------------------------
...
function PreviewModal({ rows, shippingTemplate, onClose, onPushComplete }: PreviewModalProps) {
  ...
  // Map from sku → push result for inline status column
  ...
  async function handleBulkPush() {
    // Push remaining eligible SKUs (all except the one already tested)
    ...
    <div>
      ...
      {previewOpen && previewRows.length > 0 && (
        <PreviewModal
          ...
          onPushComplete={() => { setRefreshKey(k => k + 1); setPreviewOpen(false); }}
        />
      )}
    </div>
  );
