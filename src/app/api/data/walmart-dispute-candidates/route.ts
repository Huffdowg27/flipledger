/**
 * GET   /api/data/walmart-dispute-candidates  — list with summary totals
 * PATCH /api/data/walmart-dispute-candidates  — update status (file/dismiss/received/reopen)
 */
...
export async function GET(req: NextRequest) {
  ...
  const tab = searchParams.get('tab') || 'eligible'; // eligible | maybe | filed | not_eligible | all
  ...
  const rows = db.prepare(`
    ...
    FROM walmart_dispute_candidates
    ${where}
    ORDER BY
  ...
  // Parse the JSON eligibility_reasons array on the way out
  for (const r of rows) {
  ...
  const totals = db.prepare(`
    SELECT
      ...
      SUM(CASE WHEN
        ...
        AND date(dispute_window_until) < date('now', '+7 days')
        THEN 1 ELSE 0 END) as urgent_count
    FROM walmart_dispute_candidates
  `).get() as any;

  const lastSync = (db.prepare(`SELECT value FROM settings WHERE key = 'walmart_disputes_last_sync'`).get() as { value: string } | undefined)?.value || null;
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    if (action === 'file') {
      db.prepare(`UPDATE walmart_dispute_candidates SET status = 'filed', filed_at = datetime('now'), claim_notes = ?, updated_at = datetime('now') WHERE id = ?`).run(claimNotes || null, id);
    } else if (action === 'dismiss') {
      db.prepare(`UPDATE walmart_dispute_candidates SET status = 'dismissed', claim_notes = ?, updated_at = datetime('now') WHERE id = ?`).run(claimNotes || null, id);
    } else if (action === 'received') {
      db.prepare(`UPDATE walmart_dispute_candidates SET status = 'received', updated_at = datetime('now') WHERE id = ?`).run(id);
    } else if (action === 'reopen') {
      db.prepare(`UPDATE walmart_dispute_candidates SET status = 'pending', filed_at = NULL, updated_at = datetime('now') WHERE id = ?`).run(id);
    } else {
